...
Interpretation: 
Varying X1:
Detector A: Predicted probabilities for the alpha and beta classes exhibit strong increasing and decreasing linear trends. The rho class remains mostly flat with a slight decrease as X1 increases.
Detector B: Has same trend direction as detector A, however the slopes are less steep, which is an indication of less sensitivity to X1.
Varying X2:
Detector A: The predicted probability of alpha decreases, beta increases and rho slightly increases as X2 increases.
Detector B: Has same trend direction as detector A, however slopes are still flatter than detector A, which therefore proposes lower responsiveness to changes in X2.
Overall:
The regularised model does yield different predictions for detector A and detector B. When altering input variables X1 and X2, the degree of change in predicted probabilities clearly shows these differences in predictions. Detector A displays better sensitivity, and detector B displays greater stability and less responsive curves, which is most likely as a result of stronger regularisation shrinkage.